...
class Model(object):
  ...
  def test(self, sess, uij):
    res1, att_1, stt_1 = sess.run([self.logits, self.att, self.stt], feed_dict={
        self.u: uij[0],
        self.i: uij[1],
        self.hist_i: uij[3],
        self.hist_t: uij[4],
    ...
    res2, att_2, stt_2 = sess.run([self.logits, self.att, self.stt], feed_dict={
        self.u: uij[0],
        self.i: uij[2],
        self.hist_i: uij[3],
        self.hist_t: uij[4],
    ...
    return res1, res2, att_1, stt_1, att_2, stt_2
...
def multihead_attention(
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1]))  # (h*N, T_q, T_k) +C/h
    ...
    # Activation
    outputs = tf.nn.softmax(outputs)  # (h*N, T_q, T_k) +C/h
